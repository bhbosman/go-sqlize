...
func (compiler *Compiler) projectSources(w io.Writer, tabCount int, sources []ISource) {
	if len(sources) > 0 {
		for _, source := range sources {
			...
			case JoinInformation:
				...
				compiler.internalProjectNode(w, tabCount+1, item.condition)
			}
		}
	}
}
...
func (compiler *Compiler) processGraphMap(ss []string, graphMap map[string]map[string]graph.Edge[string]) []string {
	...
	if len(graphMap) > 0 {
		return compiler.processGraphMap(noPredecessors, graphMap)
	}
	return ss
}
