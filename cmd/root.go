		...
		for _, argItem := range args {
			...
			currentContext := &internal.CurrentContext{
				internal.ValueInformationMap{
					"__stdOut__": internal.ValueInformation{
						internal.Node[ast.Node]{Node: &internal.ReflectValueExpression{
							reflect.ValueOf(cmd.OutOrStdout()),
							internal.ValueKey{"ABC", "ABC"}}}},
				},
